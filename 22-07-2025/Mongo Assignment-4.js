...
db.sessions.aggregate([
  ...
  { $project: { _id: 0, name: "$member_info.name", duration: 1 } }
])
...
db.sessions.aggregate([
  { $group: { _id: "$member_id", uniqueTrainers: { $addToSet: "$trainer_id" } } },
  { $project: { trainerCount: { $size: "$uniqueTrainers" } } },
  { $match: { trainerCount: { $gt: 1 } } }
])
...
db.sessions.aggregate([
  ...
  { $unwind: "$member" },
  { $group: { _id: "$member.membership_type", sessionCount: { $sum: 1 } } }
])
